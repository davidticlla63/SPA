...
import bo.com.erp360.webapp.data.SubDetalleFacturaRepository;
...
import bo.com.erp360.webapp.service.SubDetalleFacturaRegistration;
import bo.com.erp360.webapp.util.CodigoControl7;
import bo.com.erp360.webapp.util.UtilidadesFacturacion;

@Named(value = "facturaAlquilerController")
@ConversationScoped
public class FacturaAlquilerController implements Serializable {
	...
	Logger log = Logger.getLogger(FacturaAlquilerController.class);
		...
		try {
			...
			if (productoServicio.equals("servicio")
					&& (totalImportePorProducto == 0 || textoServicio.trim()
							.length() == 0 || periodo.trim()
							.length() == 0)) {
				FacesMessage m = new FacesMessage(FacesMessage.SEVERITY_ERROR,
						"Revisar la Cantidad o el Servicio ",
						"por favor llenar.");
				facesContext.addMessage(null, m);
				log.error("Revisar la Cantidad o el Producto, o Periodo");
				return;
			}
			DetalleFactura detalleFactura = new DetalleFactura();
			...
			detalleFactura.setPeriodo(periodo);
			detalleFactura.setPrecioTotal(totalImportePorProducto);
