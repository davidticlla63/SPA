...
public class FacturaListaController implements Serializable {
	...
	private void loadValuesDefaul() {
		...
		formatoFactura=FormatoFacturaRepository.findActivosByEmpresa(empresaLogin,sucursalLogin)
				.get(0);
	}
		...
		try {
			...
			if (formatoHoja.getNombre().equals("COMPLETO")) {
				if (formatoFactura.getNombre().equals("DOS COLUMNAS")) {
					url = urlPath + "ReportFacturaConsultora?pIdFactura=" + selectedFactura.getId()
				...
				if (formatoFactura.getNombre().equals("CUATRO COLUMNAS")) {
					url = urlPath + "ReportFactura4ColConsultora?pIdFactura=" + selectedFactura.getId()
							+ "&pEmpresa=" + empresaLogin.getRazonSocial()
							+ "&pCiudad=" + empresaLogin.getCiudad()
							+ "&pPais=BOLIVIA&pLogo=" + urlLogo + "&pNit="
							+ empresaLogin.getNit() + "&pQr="
			...
			if (formatoHoja.getNombre().equals("SIN LOGO")) {
				url = urlPath + "ReportFacturaSCF?pIdFactura="
			...
			if (formatoHoja.getNombre().equals("SIN LOGO, SIN BORDE")) {
				url = urlPath + "ReportFacturaConsultora?pIdFactura="
						+ selectedFactura.getId() + "&pEmpresa="
						+ empresaLogin.getRazonSocial() + "&pCiudad="
						+ empresaLogin.getCiudad() + "&pPais=BOLIVIA&pLogo="
						+ urlLogo + "&pNit=" + empresaLogin.getNit() + "&pQr="
